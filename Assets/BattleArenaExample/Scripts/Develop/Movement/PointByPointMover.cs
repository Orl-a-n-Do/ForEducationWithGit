...
public class PointByPointMover : MonoBehaviour
{
    ...
    private IEnumerator ProcessMove()
    {
        while(true)
        {
            ...
            float timeToMovement = (endPosition - startPosition).magnitude / _speed;
            float progress = 0;

            while(progress < timeToMovement)
            {
               transform.position   = Vector3.Lerp(startPosition, endPosition, progress/timeToMovement);  
